...
def landing(request):
    try:
        product = Stock.objects.filter(is_deleted=False).latest('created_at')
        context = {"product":product, "title":'Halaman Utama'}
        return render(request, "landing.html", context=context)
    # is_del = Stock.objects.filter(is_deleted=False).values_list("is_deleted", flat=True)[0]
    # if is_del:
